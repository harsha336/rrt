...
void MyViewer::add_model ( SnPrimitive* s, GsVec p ,int object)
{
	if(object == RRTGraph::WALL)
	{
		...
		manip->child(g);
		walls_->add(manip);
		rg_->addWall(GsPnt2(p.x+s->prim().ra+5.0f, p.y+s->prim().rb+5.0f), 
			GsPnt2(p.x-s->prim().ra+5.0f, p.y+s->prim().rb+5.0f),
			GsPnt2(p.x-s->prim().ra+5.0f, p.y-s->prim().rb+5.0f), 
			GsPnt2(p.x+s->prim().ra+5.0f, p.y-s->prim().rb+5.0f));
	}
...
void MyViewer::build_scene()
{
	...
	SnPrimitive *p;
	GsVec c1(100,0,0);
	p = new SnPrimitive(GsPrimitive::Box, 2.0f, 100.0f, 10.0f);
	p->prim().material.diffuse = GsColor::blue;
	add_model(p, GsVec(100.0f,0.0f,0.0f),RRTGraph::WALL);

        p = new SnPrimitive(GsPrimitive::Box, 100.0f, 2.0f, 10.0f);
        p->prim().material.diffuse = GsColor::blue;
        add_model(p, GsVec(0.0f,100.0f,0.0f),RRTGraph::WALL);
	...
	p = new SnPrimitive(GsPrimitive::Box, 2.0f, 2.0f, 2.0f);
	p->prim().material.diffuse = GsColor::green;
	add_model(p, GsVec(35.0f, 35.0f, 0.0f), RRTGraph::GOAL);

	p = new SnPrimitive(GsPrimitive::Sphere, 5.0f);
	p->prim().material.diffuse = GsColor::red;
	add_model(p, GsVec(10.0f,10.0f,0.0f),RRTGraph::ROBOT);

	p = new SnPrimitive(GsPrimitive::Box, 20.0f, 2.0f, 5.0f);
	p->prim().material.diffuse = GsColor::blue;
	add_model(p, GsVec(0.0f,-10.0f,0.0f),RRTGraph::WALL);

	/*p = new SnPrimitive(GsPrimitive::Box, 2, 40, 5);
        p->prim().material.diffuse = GsColor::blue;
        add_model(p, GsVec(0,-10,0),RRTGraph::WALL);*/
